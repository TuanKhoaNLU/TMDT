        ...
        <div className="detail-copy">
          ...
          <div className="detail-actions">
            ...
            <button className="btn secondary" onClick={() => wishlistMutation.mutate()} type="button">
              <Heart size={17} fill={wished ? "currentColor" : "none"} /> {wished ? "Da luu" : "Wishlist"}
            </button>
            <button className="btn secondary" onClick={() => followMutation.mutate()} type="button">
              <Store size={17} /> {followedShop ? "Dang follow shop" : "Follow shop"}
            </button>
      ...
      <section className="content-grid">
          ...
          <div className="stack-list">
            {questions.map((item) => (
              <div className="soft-row" key={item.id}>
                ...
                <span className="muted">{item.answer || "Shop chua tra loi"}</span>
              </div>
            ))}
        ...
        <article className="panel">
          ...
          >
            ...
            <input value={review} onChange={(event) => setReview(event.target.value)} placeholder="Cam nhan cua ban" />
            <button className="btn primary" type="submit"><Star size={16} /> Danh gia</button>
          </form>
          <div className="stack-list">
            {reviews.map((item) => (
              <div className="soft-row" key={item.id}>
